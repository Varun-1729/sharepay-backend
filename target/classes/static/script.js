...
function displayGroupBalances(balanceData) {
    ...
    summaryDiv.innerHTML = `
        ...
        <p><strong>Total Group Expenses: ₹${balanceData.totalGroupExpenses || 0}</strong></p>
    `;
    ...
    balanceData.userBalances.forEach(userBalance => {
        ...
        const netBalanceClass = netBalance >= 0 ? 'balance-positive' : 'balance-negative';
        const netBalanceText = netBalance >= 0 ?
            `Gets back ₹${Math.abs(netBalance).toFixed(2)}` :
            `Owes ₹${Math.abs(netBalance).toFixed(2)}`;

        balanceCard.innerHTML = `
            <h4>${userBalance.userName || 'Unknown User'}</h4>
            <p>Total Paid: ₹${totalPaid.toFixed ? totalPaid.toFixed(2) : totalPaid}</p>
            <p>Total Share: ₹${totalOwed.toFixed ? totalOwed.toFixed(2) : totalOwed}</p>
    ...
    if (balanceData.settlements && balanceData.settlements.length > 0) {
        ...
        settlementsDiv.className = 'settlements-section';
        ...
        balanceData.settlements.forEach(settlement => {
            ...
            settlementItem.innerHTML = `
                <p><strong>${settlement.fromUserName || 'Unknown'}</strong> should pay
                <span class="settlement-amount">₹${amount.toFixed ? amount.toFixed(2) : amount}</span>
                to <strong>${settlement.toUserName || 'Unknown'}</strong></p>
            `;
        ...
        settledDiv.className = 'settled-section';
